...
export function detallesTarea(tarea) {
    ...
    console.log("Dificultad:", "★".repeat(Number(tarea.dificultad)).split("").join(" "));
    ...
    if (eleccion === 1) {
        ...
        if (["P", "E", "T", "C"].includes(nuevoEstado))
            tarea.estado = nuevoEstado;
        let nuevaDificultadInput = prompt("Ingrese la nueva dificultad [1],[2],[3] (deje en blanco para mantener 1): ").trim();
        if (nuevaDificultadInput === "") {
            tarea.dificultad = "1"; // se asigna 1 si deja vacío
        }
        else if (["1", "2", "3"].includes(nuevaDificultadInput)) {
            tarea.dificultad = nuevaDificultadInput;
        }
        else {
