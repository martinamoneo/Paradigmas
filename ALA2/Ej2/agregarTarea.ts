...
export function agregarTarea(listaTareas: Tarea[]) {
    ...
    // 3. Estado
    let estado: "P" | "E" | "T" | "C" = "P"; // valor inicial válido
    while(!["P", "E", "T", "C"].includes(estado)) {
        estado = prompt("3. Estado ([P] Pendiente / [E] En curso / [T] Terminada / [C] Cancelada): ").toUpperCase() as "P" | "E" | "T" | "C";
    ...
    // 4. Dificultad
    let dificultad: "1" | "2" | "3" = "1"; // valor inicial válido
    while(!["1","2","3"].includes(dificultad)) {
        dificultad = prompt("4. Dificultad ([1] Fácil / [2] Media / [3] Difícil): ") as "1" | "2" | "3";
